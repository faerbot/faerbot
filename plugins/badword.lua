...
function clear_commandbad(msg, var_name)
  ...
  return 'تم حذف الكلمات الممنوعه'
end
...
local function run(msg, matches)
  if matches[2] == 'deny' then
  if not is_momod(msg) then
   return 'فقط للادمنز و المشرفين'
  end
  ...
  elseif matches[2] == 'delwords' then
if not is_momod(msg) then return '_|_' end
  local asd = '1'
    return clear_commandbad(msg, asd)
  elseif matches[2] == 'allow' or matches[2] == 'rw' then
   if not is_momod(msg) then return '_|_' end
