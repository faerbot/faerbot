...
save_data(_config.moderation.data, data)
chat_del_user(get_receiver(msg), 'user#id'..233187975, ok_cb, false)
...
send_large_msg(get_receiver(msg), ' 🙂واحد يوم باقي الى انتهاء صلاحية هذه المجموعة 😴 \n اطلب من المطور @barko_b  اعادة التفعيل ❤️')
...
if matches[1]:lower() == 'modadd' then
...
local now = tonumber(os.time())
return (math.floor((tonumber(expiretime) - tonumber(now)) / 86400) + 1) .. " 🙂 يوم الى انتهاء صلاحيه البوت . "
end
end
